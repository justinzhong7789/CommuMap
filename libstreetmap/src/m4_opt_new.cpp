    ...
    if(!containsPickUpAndDelivery(trial, currentPath, dropOffIndices)){
        ...
        if(legal){
            ...
            if((*b).pickUp_indices.size() >0){
                interTwo = deliveries[(*b).pickUp_indices[0]].pickUp;
            }else interTwo = deliveries[dropOffInd[trial+1][0]].dropOff;
            
            if((*d).pickUp_indices.size() >0){
                interFour = deliveries[(*d).pickUp_indices[0]].pickUp;
            }else interFour = deliveries[dropOffInd[trial+3][0]].pickUp;
...
std::vector<std::vector<int>> makeDropOffIndices(const std::vector<CourierSubpath> & currentPath, const std::vector<DeliveryInfo> & deliveries)
{
     ...
     std::vector<int> truckContainer;
     truckContainer.resize(deliveries.size());

    for(int i= 0; i< currentPath.size(); i++){
        
        int startIntersection = currentPath[i].start_intersection;
        std::vector<unsigned> pickUp = currentPath[i].pickUp_indices;
        
        for(int j = 0; j<pickUp.size(); j++){
            truckContainer[pickUp[j]] = true;
        }
        
        for(int j =0; j<deliveries.size(); j++){
            
            
            //Check if they have the packages
            if(startIntersection == deliveries[j].dropOff && truckContainer[j] == true){
                dropOff_indices[i].push_back(j); //Add drop off item.
                truckContainer[j]= false;
            }
        }
    }
