...
//    if(tracebackTemp.size()!=0){tracebackTemp.clear();}
//    
//    for(int i=0;i<destinations.size();i++){
//        tracebackTemp.push_back(bfsTraceback(destinations[i]));
//    }
//}
//bool one_source_multi_dest(Node* sourceNode, vector<IntersectionIndex> destinations, double turn_penalty){
//        // Establish min heap
...
//        if (all_path_found){
//            for (int j=0; j < PU_loc.size(); j++){
//                list<int> temp = tracebackTemp[j];
//                path to_one_PU = convertListToVec(temp);
//                pathInfo one_path(to_one_PU, compute_path_travel_time(to_one_PU, turn_penalty));
//                one_depot_to_all_PU.push_back(one_path);
//            }
//            tracebackTemp.clear();
...
//        else {return false;}*/
//
////        all_PU_to_other_points.push_back(one_depot_to_all_pu);
//        //reset_nodeTable();
//    }
//    return true;  
//}
...
//    bool make_depot_to_pu = make_depot_to_PU(deliveries, depots, turn_penalty);
//    bool make_pu_to_other = make_PU_to_other_points(deliveries, turn_penalty);
//    bool make_do_to_point = make_DO_to_points(deliveries,depots, turn_penalty);
//    
//    return (make_depot_to_pu && make_pu_to_other && make_do_to_point);
//}
...
//{
//    vector<CourierSubpath> stupidPath ; 
//    int deliNum = deliveries.size();
//    CourierSubpath begin = initializeSubpath(depots[0], deliveries[0].pickUp, depots_to_all_PU[0][0].the_path,{});
//    stupidPath.push_back(begin);
//    for(int i=0;i<deliveries.size()-1;i++){
//        //from pickup[i] to drop off[i]
//        CourierSubpath subpath = initializeSubpath(deliveries[i].pickUp, deliveries[i].dropOff, all_PU_to_other_points[i][i].second.the_path, {i});
//        stupidPath.push_back(subpath);
//        //from drop off[i] to pickup[i+1]
//        subpath = initializeSubpath(deliveries[i].dropOff, deliveries[i+1].pickUp , all_DO_to_other_points[i][i+1].first.the_path, {});
//        stupidPath.push_back(subpath);
//    }
//    // from last pickup to last drop off
//    CourierSubpath subpath = initializeSubpath(deliveries[deliNum- 1].pickUp, deliveries[deliNum-1].dropOff, all_PU_to_other_points[deliNum-1][deliNum-1].second.the_path, {deliNum -1});
//    stupidPath .push_back(subpath);
//    //from final drop off to depot0
//    subpath = initializeSubpath(deliveries[deliNum -1].dropOff, depots[0], all_DO_to_depots[deliNum-1][0].the_path, {});
//    stupidPath.push_back(subpath);
//    return stupidPath;
//    
//}
...
//        return stupid_path(deliveries, depots, turn_penalty, truck_capacity);
//        /*
...
//            }
//        }
//        CourierSubpath begin = initializeSubpath(depots[startdepotIndex], getEndInter(closest_path.the_path), closest_path.the_path,{});
//        best.push_back(begin);
//    }
...
//}
//
//CourierSubpath initializeSubpath(IntersectionIndex begin, IntersectionIndex end, path path_, vector<unsigned> pickupIndex){
//    CourierSubpath a;
//    a.start_intersection = begin;
