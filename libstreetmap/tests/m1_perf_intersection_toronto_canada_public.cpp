...
SUITE(intersection_queries_perf_public_toronto_canada) {
    ...
    TEST_FIXTURE(MapFixture, intersection_street_segments_perf) {
        ...
        std::vector<IntersectionIndex> intersection_ids;
        for(size_t i = 0; i < 15000000; i++) {
            intersection_ids.push_back(rand_intersection(rng));
        }
        {
            //Timed Test
            ECE297_TIME_CONSTRAINT(2055);
            std::vector<StreetSegmentIndex> result;
            for(size_t i = 0; i < 15000000; i++) {
                result = find_street_segments_of_intersection(intersection_ids[i]);
            }
        }
    ...
    TEST_FIXTURE(MapFixture, intersection_street_names_perf) {
        ...
        {
            //Timed Test
            ECE297_TIME_CONSTRAINT(1264);
            std::vector<std::string> result;
            for(size_t i = 0; i < 1000000; i++) {
                result = find_street_names_of_intersection(intersection_ids[i]);
            }
        }
    ...
    TEST_FIXTURE(MapFixture, directly_connected_perf) {
        ...
        //Generate random inputs
        std::vector<IntersectionIndex> intersection_ids_1;
        std::vector<IntersectionIndex> intersection_ids_2;
        ...
        {
            //Timed Test
            ECE297_TIME_CONSTRAINT(1158);
            bool result;
            for(size_t i = 0; i < 2000000; i++) {
                result &= are_directly_connected(std::make_pair(intersection_ids_1[i], intersection_ids_2[i]));
            }
        }
