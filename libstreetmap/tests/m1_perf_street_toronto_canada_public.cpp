...
SUITE(street_queries_perf_public_toronto_canada) {
    ...
    TEST_FIXTURE(MapFixture, street_street_segments_perf) {
        ...
        std::vector<StreetIndex> street_ids;
        for(size_t i = 0; i < 8000000; i++) {
            street_ids.push_back(rand_street(rng));
        }
        {
            //Timed Test
            ECE297_TIME_CONSTRAINT(1038);
            std::vector<StreetSegmentIndex> result;
            for(size_t i = 0; i < 8000000; i++) {
                result = find_street_segments_of_street(street_ids[i]);
            }
        }
    ...
    TEST_FIXTURE(MapFixture, all_street_intersections_perf) {
        ...
        {
            //Timed Test
            ECE297_TIME_CONSTRAINT(1763);
            std::vector<IntersectionIndex> result;
            for(size_t i = 0; i < 1600000; i++) {
                result = find_intersections_of_street(street_ids[i]);
            }
        }
    ...
    TEST_FIXTURE(MapFixture, intersection_ids_from_street_ids_perf) {
        ...
        //Generate random inputs
        std::vector<StreetIndex> street_ids_1;
        std::vector<StreetIndex> street_ids_2;
        for(size_t i = 0; i < 900000; i++) {
            StreetIndex street_id1 = rand_street(rng);
            StreetIndex street_id2 = rand_street(rng);
            street_ids_1.push_back(street_id1);
            street_ids_2.push_back(street_id2);
        }
        {
            //Timed Test
            ECE297_TIME_CONSTRAINT(521);
            std::vector<IntersectionIndex> result;
            for(size_t i = 0; i < 900000; i++) {
                result = find_intersections_of_two_streets(std::make_pair(street_ids_1[i], street_ids_2[i]));
            }
        }
    ...
    TEST_FIXTURE(MapFixture, street_ids_from_partial_street_name_perf) {
        ...
        {
            //Timed Test
            ECE297_TIME_CONSTRAINT(24810);
            std::vector<StreetIndex> result;
            for(size_t i = 0; i < 1000000; i++) {
